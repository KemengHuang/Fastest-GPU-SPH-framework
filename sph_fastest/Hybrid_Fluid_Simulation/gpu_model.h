...
{
    //float l2_hit_rate;          // L2 cache hit rate, get from NSIGHT profiler
    //float num_uncoal_per_warp;  // #memory_transactions per warp(uncoalesced access)
    ...
    //float block_size;           // #threads in a CTA
};

struct GPUDeviceInfo
{
    //float freq;                 // GPU core clock frequency in GHz
    //float mem_peak_bw;          // GPU memory bandwidth in GBps
    float simd_width;           // #SPs per SM
    float warp_size;            // #threads in a warp
    //float transaction_size;     // transaction size for a DRAM request in Bytes
    //float l2_lat;
